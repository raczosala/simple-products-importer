...
class Simple_Products_Importer_List extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct( [
			'singular' => __( 'Entry Name', 'simple-products-importer' ),
			'plural'   => __( 'Entry Names', 'simple-products-importer' ),
			'ajax'     => false
		] );
	}
	...
	public static function get_entries( $per_page = 5, $page_number = 1 ) {
		global $wpdb;
		$sql = 'SELECT * FROM ' . Simple_Products_Importer_DB::get_table_name();
	...
	public static function delete_entry( $id ) {
		global $wpdb;
		$wpdb->delete(
			Simple_Products_Importer_DB::get_table_name(),
			[ 'id' => $id ],
			[ '%d' ]
		);
	}
	...
	public static function record_count() {
		global $wpdb;
		$sql = 'SELECT COUNT(*) FROM ' . Simple_Products_Importer_DB::get_table_name();
	...
	public function no_items() {
		_e( 'No entries found.', 'simple-products-importer' );
	}
	...
	function column_title($item){
		...
		$actions = array(
			'edit'      => sprintf('<a href="?page=%s&action=edit&id=%s">%s</a>', 'simple-products-importer-entries-view', $item['id'], __('Edit')),
		...
		return sprintf('<a href="?page=%1$s&id=%2$s" class="row-title">%3$s</a> %4$s',
			/*$1%s*/ 'simple-products-importer-entry-edit',
			/*$2%s*/ $item['id'],
			/*$3%s*/ $item['title'],
	...
	function get_columns() {
		$columns = [
			...
			'title'    => __( 'Title', 'simple-products-importer' ),
			'id' => __( 'Entry ID', 'simple-products-importer' ),
			'shortcode'    => __( 'Shortcode', 'simple-products-importer' )
		];
		return $columns;
	}
